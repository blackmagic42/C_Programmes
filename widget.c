...
rect InsidePart(rect root,rect other){
    ...
    It return the part of the other rectangle in the root rectangle.*/
    rect r;
    int width,height,w1,w2,h1,h2;
    r.x=MAX(root.x,other.x);
    r.y=MAX(root.y,other.y);
    
    w2=root.x+root.width-r.x;
    r.x-=other.x;
    w1=other.width-r.x;

    h2=root.x+root.height-r.y;
    r.y-=other.y;
    h1=other.height-r.y;

    width=MIN(w1,w2);
    height=MIN(h1,h2);
    if (width<=0 || height<=0) r.height=r.width=0;
    else{
        r.width=width;
        r.height=height;
    }
